...
from auto_vdp_procedure import AutoVdpProcedure
...
class MainWindow(ManagedDockWindow):
    ...
    def __init__(self):
        ...
        boxes = ['atlas_addr', 'study_id', 'wafer_id','map_1_chip_id','map_2_chip_id', 'map_3_chip_id',
                 'vdp_start_current', 'vdp_stop_current', 'vdp_current_num','vdp_delay', 
                 'gate_start_voltage', 'gate_stop_voltage', 'gate_voltage_num']
        
        
        super(MainWindow, self).__init__(
            procedure_class=AutoVdpProcedure,
            inputs=boxes,
            displays=boxes,

            x_axis=['timestamp','timestamp','timestamp'],
            y_axis=['VdP Current (A)','VdP Voltage (V)', 'Resistance (ohm)'],
        ...
        self.setWindowTitle('Automated VdP')
    ...
    def queue(self, procedure=None):
        ...
        # Naming File
        if study_id =='S1':
            directory = r"Z:\projmon\vdp-fet-board-data\Study 1\\" + study_id + "_" + wafer_id + '\\vdp\\raw_files'       
        else:
