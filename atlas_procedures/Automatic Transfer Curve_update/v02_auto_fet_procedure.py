...
class v02_AutoFETProcedure(Procedure):
    ...
    def execute(self):
        ...
        if self.V_g_start < self.V_g_end:
            V_g_step = self.V_g_sweep_rate*0.19009 #0.019009 is the time between each step
        elif self.V_g_start > self.V_g_end:
            V_g_step = -self.V_g_sweep_rate*0.19009
        voltage_forward = np.arange(self.V_g_start, self.V_g_end, V_g_step)
        voltage_reverse = np.arange(self.V_g_end, self.V_g_start, -V_g_step)
        V_g_voltages = np.concatenate((voltage_forward, voltage_reverse))
        ...
        for step, V_g in enumerate(V_g_voltages):
            ...
            self.emit('progress', 100.*(step+1)/len(V_g_voltages))
