...
class v02_AutoVdpProcedure_SweepVg(Procedure):
    ...
    VdP_voltage_range = FloatParameter('Voltage Range (VdP)', units='V', default=20)
    G_current_range = FloatParameter('Current Range (Gate)', units='A', default=1E-7)
    ...
    def execute(self):
        ...
        # Compiling one voltage sweep
        if self.gate_voltage_start < self.gate_voltage_end:
            gate_voltage_step = self.gate_voltage_sweep_rate*0.019009 #0.019009 is the time between each step
        elif self.gate_voltage_start > self.gate_voltage_end:
            gate_voltage_step = -self.gate_voltage_sweep_rate*0.019009
        gate_voltage_forward = np.arange(self.gate_voltage_start, self.gate_voltage_end, gate_voltage_step)
        gate_voltage_reverse = np.arange(self.gate_voltage_end, self.gate_voltage_start, -gate_voltage_step)
        gate_voltages = np.concatenate((gate_voltage_forward, gate_voltage_reverse))
